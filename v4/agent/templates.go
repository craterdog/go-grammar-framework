...
type ParserLike interface {
	...
	// Methods
	ParseSource(source string) ast.<Class>Like
}
...
type ValidatorLike interface {
	...
	// Methods
	Validate<Class>(<parameter> ast.<Class>Like)
}
`
...
func (v *parser_) ParseSource(source string) ast.<Class>Like {
	...
	var <class>, token, ok = v.parse<Class>()
	if !ok {
...
var syntax = map[string]string{
	"<Package>": "<Class> EOL* EOF  ! Terminated with an end-of-file marker.",
}
`

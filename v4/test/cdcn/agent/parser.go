...
import (
	...
	ast "github.com/craterdog/go-grammar-framework/v4/test/cdcn/ast"
	sts "strings"
)
...
func (v *parser_) ParseSource(source string) ast.CollectionLike {
	// The scanner runs in a separate Go routine.
	v.source_ = source
	Scanner().Make(v.source_, v.tokens_)

	// Attempt to parse a model.
	var model, token, ok = v.parseCollection()
	if !ok {
...
var syntax = map[string]string{
	"Agent": "Collection EOL* EOF  ! Terminated with an end-of-file marker.",
}

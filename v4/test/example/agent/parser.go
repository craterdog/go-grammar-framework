...
import (
	...
	ast "github.com/craterdog/go-grammar-framework/v4/test/example/ast"
	sts "strings"
)
...
func (v *parser_) ParseSource(source string) ast.RootLike {
	// The scanner runs in a separate Go routine.
	v.source_ = source
	Scanner().Make(v.source_, v.tokens_)

	// Attempt to parse a model.
	var model, token, ok = v.parseRoot()
	if !ok {
		var message = v.formatError(token)
		message += v.generateSyntax("Root",
			"Agent",
			"Root",
		)
		panic(message)
...
func (v *parser_) parseRoot() (
	root ast.RootLike,
	token TokenLike,
	ok bool,
) {
	// TBA - Add real method implementation.
	return root, token, ok
}
...
var syntax = map[string]string{
	"Agent": "Root EOL* EOF  ! Terminated with an end-of-file marker.",
}

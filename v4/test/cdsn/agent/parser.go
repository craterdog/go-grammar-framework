...
import (
	...
	ast "github.com/craterdog/go-grammar-framework/v4/test/cdsn/ast"
	sts "strings"
)
...
func (v *parser_) ParseSource(source string) ast.SyntaxLike {
	...
	var model, token, ok = v.parseSyntax()
	if !ok {
...
var syntax = map[string]string{
	"Agent": "Syntax EOL* EOF  ! Terminated with an end-of-file marker.",
}

...
func (v *parser_) ParseSource(source string) ExampleLike {
	// The scanner runs in a separate Go routine.
	v.source_ = source
	Scanner().Make(v.source_, v.tokens_)

	// Attempt to parse a model.
	var model, token, ok = v.parseExample()
	if !ok {
		var message = v.formatError(token)
		message += v.generateGrammar("Example",
			"Test",
			"Example",
		)
		panic(message)
...
var grammar = map[string]string{
	"Test": "Example EOL* EOF  ! Terminated with an end-of-file marker.",
}
